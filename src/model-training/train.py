...
# ------------------------------------------------------------ #
    ...
    y_true = test_data[args.init_args['label']]
    report_dict = classification_report(y_true, y_pred, output_dict=True, labels=predictor.class_labels)
    report_dict_df = pd.DataFrame(report_dict).T
    report_dict_df.to_csv(f"{args.output_data_dir}/classification_report.csv", index=True)

    # Draw confusion matrix
    cm_labels = ['AAA', 'AA', 'A', 'BBB', 'BB', 'B', 'CCC']
    ...
    plt.show()
    plt.savefig(f"{args.output_data_dir}/confusion_matrix.png")
